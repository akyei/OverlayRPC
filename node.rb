...
puts("ruby /home/core/OverlayRPC2/graph.rb -w #{weightfile} -r #{routefile} -d #{dumpinterval} -f #{routeinterval} -m #{length}")
fork do
exec("ruby /home/core/OverlayRPC2/graph.rb -w #{weightfile} -r #{routefile} -d #{dumpinterval} -f #{routeinterval} -m #{length} ")
end
fork do
